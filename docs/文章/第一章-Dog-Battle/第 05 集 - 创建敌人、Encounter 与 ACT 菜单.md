...
三行配置，三件事：开场文本、战斗音乐、敌人名单。先看第一行——`Game:loc("key")` 是取文本的 API：按当前语言，返回语言文件里对应 key 的文本，游戏里切语言它跟着换。这是"代码侧"的取法：需要把文本拿进代码变量时用它。开场文本 `encounter_dog_start` 在语言文件里，中英各玩各的双关：
...
`cutscene:text(文本key, 表情, 角色)`——表情和角色指定谁在说话、什么表情；注意第一个参数：写的是 `"{battle_dog_tell_joke_1}"`——**花括号直接包着 key**。这就是 {key} 内联引用：写台词的位置上直接写翻译 key，台词渲染时自动替换成当前语言的文本。和 `Game:loc` 的分工是：后者在代码里"取文本当变量"，前者在台词位置上"引用文本"。{key} 也是最接近 Kristal 原生写法的形式——Kristal 原本就是在这里直接写文本的，现在把文本换成 {key}，翻译系统就接管了。整段笑话的文案：大家一起想了一个烂双关笑话……苏西（紧张）："呃……为什么狗不能当歌手？"兰博（脸红）："因为它会'汪'词！"苏西："……"旁白："狗狗完全听不懂！"——两句旁白之间还嵌着 `[sound:mus_rimshot]`（经典的讲完笑话鼓点），以及 `[react:susie_reaction]` 触发苏西的反应。
