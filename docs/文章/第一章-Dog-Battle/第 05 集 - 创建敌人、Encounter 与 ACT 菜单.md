...
但有一个模板自带的默认行为要心里有数：**你直接对敌人用 MERCY，是会给敌人加饶恕值的**——这是引擎的默认机制：饶恕值没满时，每次 MERCY 都会按敌人的 `spare_points` 给它加一点（模板里是 20）。也就是说，光靠反复 MERCY，也能把饶恕值一点点攒上去。本集一个字都不用写，默认行为先这样。

怎么才能饶恕这只狗、战斗怎么算赢，是第 09 集"胜负流程"的事。本集别动手，但别把它漏了：MERCY 是那五个选项里迟早要接的一环。
