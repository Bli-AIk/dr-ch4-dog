...
理由先说清楚：Kristal 的 ACT 不会自动送饶恕值，送不送全看你的 onAct 里写不写 `addMercy`。模板的 dummy 写了（微笑 +100、X-Action +50）——但那是模板的教学演示。我们写狗的 ACT 时，一行加值都没写：**狗的 ACT 就目前而言，不给饶恕值**。这也是本项目对 MERCY 唯一的改动。
